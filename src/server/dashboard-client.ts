export const dashboardClient = String.raw`
  (function () {
    ...
    function renderReviewerResult(detail) {
      ...
      var html = '<section class="drawer-section"><h3>Terminal result</h3><div class="panel side-card"><div style="margin-bottom:10px">' + badge(status === 'incomplete' || status === 'skipped' ? status : firstDefined(result.verdict, status)) + '</div>' + (status === 'incomplete' && result.verdict ? '<p class="secondary">Retained structured result; not accepted for clearance.</p>' : '') + '<div class="summary-copy">' + escapeHtml(summary) + "</div></div></section>";
