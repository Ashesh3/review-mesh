  ...
  registry.register("codex", () => ({
    ...
    async *run(input): AsyncIterable<AdapterEvent> {
      ...
      if (input.reviewer.policy?.mode === "adjudication") {
        ...
        const downgraded =
          (options.downgrade && input.reviewer.modelIndex === 4) ||
          options.downgradeAt?.includes(input.reviewer.modelIndex ?? -1) ===
            true;
        const rejected =
...
it("retains a confirmed gate finding after repeated advisory adjustments", async () => {
  const { result, received } = await run({
    profile: "strict-evaluation",
    finding: true,
    downgradeAt: [1, 2, 4],
